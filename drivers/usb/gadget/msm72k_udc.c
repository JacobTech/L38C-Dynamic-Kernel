...
//                                                  
	...
	unsigned is_manual = 0;

	char *envp[] = { "HOME=/", "TERM=linux", NULL, };
	char *argv[] = { "/system/bin/stop", "adbd", NULL, };
	char *argv_adb[] = { "/system/bin/start", "adbd", NULL, };
#endif
//                                                  
	struct msm_otg *otg = to_msm_otg(ui->xceiv);
...
//                                                  
#ifdef CONFIG_LGE_USB_GADGET_DRIVER
	flags = lge_is_factory_cable(&udc_cable, &is_manual);
	pr_info(" *** lge_get_cable_info : %d, %d\n", udc_cable, is_manual);

	if (flags) // factory cable
	{
		// 4.7V defence code
		if (udc_cable >= 7) {
		...
		// change usb mode
		if (android_lge_get_current_pid() != LGE_FACTORY_PID)
		{
			// Disable NMEA Port
			call_usermodehelper(argv[0], argv, envp, UMH_WAIT_PROC);

			spin_lock_irqsave(&ui->lock, flags);
			android_lge_switch_driver(LGE_FACTORY_PID, LGE_NO_RESET);
			spin_unlock_irqrestore(&ui->lock, flags);
		}
	}
	else
	{
		// change usb mode to android driver
		if (android_lge_get_current_pid() == LGE_FACTORY_PID)
		{
			// enable ADB
			call_usermodehelper(argv[0], argv, envp, UMH_WAIT_PROC);
			call_usermodehelper(argv_adb[0], argv_adb, envp, UMH_WAIT_PROC);

			spin_lock_irqsave(&ui->lock, flags);
			android_lge_switch_driver(LGE_DEFAULT_PID, LGE_NO_RESET);
			spin_unlock_irqrestore(&ui->lock, flags);
		}
	}
#endif
