...
static void gs_write_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	default:
		/* presumably a transient fault */
		// LGFW: 2012.07.16 daewon.seo - gs_write log off becaus too much log print in kernel log during ATS test
		#if 1
		pr_info(" *** %s: [!] unexpected %s status %d\n", __func__, ep->name, req->status);
		#endif
		/* FALL THROUGH */
	case 0:
		/* normal completion */
...
static int gs_write(struct tty_struct *tty, const unsigned char *buf, int count)
{
	...
	// LGFW: 2012.07.16 daewon.seo - gs_write log off becaus too much log print in kernel log during ATS test
	#if 1
	pr_info(" *** gs_write: ttyGS%d (%p) writing %d bytes\n", port->port_num, tty, count);
	#endif
