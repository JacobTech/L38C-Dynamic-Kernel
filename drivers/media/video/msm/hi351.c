...
static long hi351_reg_init(void)
{
	...
	common_register_init(COMMON_REG_REG, &pstRegisterList);
#if 0		//NORMAL MODE
	int32_t rc = 0;
	...
	if(pstRegisterList)
	{
		printk("reg_init read file\n");	
			
		ptr_list = pstRegisterList;	

		for (loop = 0; loop < ptr_list->num_regs; loop++)	
		{			
			if (ptr_list->list_regs[loop].reg.addr == 0xFF)	
			{			
				msleep(ptr_list->list_regs[loop].reg.val);		
				CDBG("[icebox]%s:msleep addr = %x, val = %x\n", __func__, ptr_list->list_regs[loop].reg.addr, ptr_list->list_regs[loop].reg.val);				rc = 0;	
			}			
			else
			{
				rc = hi351_i2c_write_b_sensor(ptr_list->list_regs[loop].reg.addr, ptr_list->list_regs[loop].reg.val);	
				CDBG("[icebox]%s: addr = %x, val = %x\n", __func__, ptr_list->list_regs[loop].reg.addr, ptr_list->list_regs[loop].reg.val);	
			}
