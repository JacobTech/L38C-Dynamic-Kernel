...
#include "MTH_LE0S_R00_V51.c" //for Suntel touch panel
...
/*                                                                                         */ 
void mms100_download(int mms100vendor)
{
	int ret =0;
	int i;
...
#else
	ret = mms100_ISC_download_binary_data(mms100vendor);
	ret = 0;
	if (ret)
	{
...
/*                                                                                                        */
			...
			if(!ret)
      {
	      ucInitial_Download = TRUE;
				ret = mms100_ISC_download_binary_data(mms100vendor);    //retry ISC mode download
      }
...
int mms100_ISC_download_binary_data(int mms100vendor2)
{
  ...
  INT8 version_info = 0;
    ...
    //---------------------------------
    if(mms100vendor2)
       	firmware_ver = MELFAS_DOWNLAOD_PUBLIC_VERSION_FOR_SUNTEL;
    else
		firmware_ver = MELFAS_DOWNLAOD_PUBLIC_VERSION_FOR_TOVIS;
	...
	/*                                                                                         */
			...
			if (iValue != MANUAL_DOWNLOAD_ENABLE) {
			  ...
			  printk("<MELFAS> CORE_VERSION : 0x%2X\n",version_info);
				...
				if(version_info < firmware_ver || version_info==0xFF)
			  	dl_enable_bit |= 0x01;
			} else {
				dl_enable_bit |= 0x01;
			}
    ...
    for(i=0;i<3;i++)
    {
        if(dl_enable_bit & (1<<i))
        {
          ...
          {
     		if(mms100vendor2)	
				nRet = mms100_ISC_download( (const UINT8*) MELFAS_binary2, (const UINT16)MELFAS_binary_nLength2, (const INT8)i);//For Suntel panel
			else
			    nRet = mms100_ISC_download( (const UINT8*) MELFAS_TS_binary, (const UINT16)MELFAS_TS_binary_nLength, (const INT8)i);//For Tovis panel
              ...
              ucInitial_Download = FALSE;
					...
					{
			if(mms100vendor2)	
				nRet = mms100_ISC_download( (const UINT8*) MELFAS_binary2, (const UINT16)MELFAS_binary_nLength2, (const INT8)i);
			else
