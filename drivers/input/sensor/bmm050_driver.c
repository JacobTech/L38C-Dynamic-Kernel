...
static ssize_t bmm_old_x_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t bmm_old_y_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", bmm_sensordata[1]);
}
static ssize_t bmm_old_z_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", bmm_sensordata[2]);
}
static ssize_t bmm_old_x_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int value;
	...
	sscanf(buf, "%d", &value);
	bmm_sensordata[0]=value;
	return count;
}
static ssize_t bmm_old_y_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int value;
	...
	sscanf(buf, "%d", &value);
	bmm_sensordata[1]=value;
	return count;
}
static ssize_t bmm_old_z_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int value;
