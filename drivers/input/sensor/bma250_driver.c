...
static int bma250_sensordata[3]={0,};
/*                                              */
		...
		const char *buf, size_t count)
{
	...
	struct i2c_client *client = to_i2c_client(dev);
	struct bma250_data *bma250 = i2c_get_clientdata(client);
	printk(KERN_INFO "bma250_selftest start!\n");
	error = strict_strtoul(buf, 10, &data);
	if (error)
	{
		printk(KERN_INFO "strict_strtoul error!\n");
		return error;
	}
	...
	{
		printk(KERN_INFO "bma250_set_range error\n");
		return -EINVAL;
	}		
...
static ssize_t bma250_old_x_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t bma250_old_y_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", bma250_sensordata[1]);
}
static ssize_t bma250_old_z_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", bma250_sensordata[2]);
}
static ssize_t bma250_old_x_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int value;
	...
	sscanf(buf, "%d", &value);
	bma250_sensordata[0]=value;
	return count;
}
static ssize_t bma250_old_y_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int value;
	...
	sscanf(buf, "%d", &value);
	bma250_sensordata[1]=value;
	return count;
}
static ssize_t bma250_old_z_store(struct device *dev,
		struct device_attribute *attr,
		const char *buf, size_t count)
{
	int value;
		...
		const char *buf, size_t count)
{
	...
	if(value == 1){
		...
		bma250_sensordata[0]=0;
		bma250_sensordata[1]=0;
		bma250_sensordata[2]=0;
	}else{
...
static DEVICE_ATTR(z, S_IRUGO|S_IWUSR|S_IWGRP,
		bma250_z_show, NULL);
static DEVICE_ATTR(old_x, S_IRUGO|S_IWUSR|S_IWGRP,
		bma250_old_x_show, bma250_old_x_store);
static DEVICE_ATTR(old_y, S_IRUGO|S_IWUSR|S_IWGRP,
		bma250_old_y_show, bma250_old_y_store);
static DEVICE_ATTR(old_z, S_IRUGO|S_IWUSR|S_IWGRP,
		bma250_old_z_show, bma250_old_z_store);
