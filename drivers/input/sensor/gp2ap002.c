...
/* Proximity Soft Shutdown Support */
...
/*                                                                                           */
	...
	if (gp2ap_pdev->op_mode == PROX_OPMODE_A)
	{
		if (onoff) {
			rc = prox_i2c_write(0x01, 0x08, PROX_I2C_INT_NO_CLEAR); /* address:1 data:0x08 LED driver's MOS pararelled */ 
	 		...
	 		rc = prox_i2c_write(0x03, gp2ap_cycle_table[gp2ap_pdev->cycle].val, PROX_I2C_INT_NO_CLEAR); /* address:3 data:0x04 8ms cycle, no Freq.Hopping */
			if (gp2ap_pdev->methods) 	/* Interrnupt mode */
				rc = prox_i2c_write(GP2AP_REG_OPMOD, 0x03, GP2AP_NO_INTCLEAR); /* address:4 data:0x01 SSD:b'1 VCON:b'0 */
			else				/* Normal mode */
				rc = prox_i2c_write(GP2AP_REG_OPMOD, 0x01, GP2AP_NO_INTCLEAR); /* address:4 data:0x01 SSD:b'1 VCON:b'0 */

	  		//rc = prox_i2c_write(0x04, 0x01, PROX_I2C_INT_NO_CLEAR); /* address:4 data:0x01 SSD:b'1 VCON:b'0 */
			if (rc < 0) {
	  			pr_err("%s: operation_enable failed (%d)\n", __func__, rc);
	 		}
	 	} else {
	    		rc = prox_i2c_write(0x04, 0x00, PROX_I2C_INT_NO_CLEAR); /* address:4 data:0x00 SSD:0 VCON:b'0 */
			if (rc < 0) {
	...
	{
		if (onoff) {
			rc = prox_i2c_write(GP2AP_REG_GAIN, 0x08, GP2AP_NO_INTCLEAR); /* address:1 data:0x08 LED driver's MOS pararelled */ 
			...
			rc = prox_i2c_write(GP2AP_REG_CYCLE, gp2ap_cycle_table[gp2ap_pdev->cycle].val, GP2AP_NO_INTCLEAR); /* address:1 data:0x08 LED driver's MOS pararelled */ 
//	  		rc = prox_i2c_write(GP2AP_REG_OPMOD, 0x03, GP2AP_NO_INTCLEAR); /* address:4 data:0x01 SSD:b'1 VCON:b'0 */
			if (gp2ap_pdev->methods) 	/* Interrnupt mode */
			{
				rc = prox_i2c_write(GP2AP_REG_OPMOD, 0x03, GP2AP_NO_INTCLEAR); /* address:4 data:0x01 SSD:b'1 VCON:b'0 */	
				...
				rc = prox_i2c_write(GP2AP_REG_OPMOD, 0x01, GP2AP_NO_INTCLEAR); /* address:4 data:0x01 SSD:b'1 VCON:b'0 */
			rc = prox_i2c_write(GP2AP_REG_CON, 0x00, GP2AP_INTCLEAR);	
...
clear_interrupt:
	if (dev->methods) {	/* Interrupt mode */
		if (dev->vout_level == 0) {
			if (dev->op_mode == PROX_OPMODE_A) {
				prox_i2c_write(GP2AP_REG_HYS, 0xC2, GP2AP_NO_INTCLEAR);
			} else if (dev->op_mode == PROX_OPMODE_B1) {
...
static ssize_t
gp2ap_opmode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct proximity_gp2ap_device *pdev = i2c_get_clientdata(client);
	printk(KERN_INFO "gp2ap_opmode_store() is started.\n");
	prox_power_onoff(0);
	sscanf(buf, "%d", &pdev->op_mode);
	udelay(20);
	prox_power_onoff(1);	
	udelay(120);
	printk(KERN_INFO "gp2ap_opmode_store() is completed.\n");
	return count;
}
...
/*                                                             */
	...
	__ATTR(opmod, S_IRUGO | S_IWUSR, gp2ap_opmode_show, gp2ap_opmode_store),
/*                                                             */
