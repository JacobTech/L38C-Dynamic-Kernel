...
static int adreno_start(struct kgsl_device *device, unsigned int init_ram)
{
	...
	*/
	...
	if (status != 0)
		goto error_irq_off;
#if 0 //deleting log 06-15 sohyun.nam
	KGSL_PWR_ERR(device, "device started status %d ts %x\n", status, adreno_dev->ringbuffer.timestamp);
#endif
...
static int adreno_stop(struct kgsl_device *device)
{
	...
	kgsl_pwrctrl_disable(device);
#if 0 //deleting log 06-15 sohyun.nam
	KGSL_PWR_ERR(device, "device stopped ts %x\n", adreno_dev->ringbuffer.timestamp);
#endif
	return 0;
}
...
int adreno_idle(struct kgsl_device *device, unsigned int timeout)
{
	struct adreno_device *adreno_dev = ADRENO_DEVICE(device);
	struct adreno_ringbuffer *rb = &adreno_dev->ringbuffer;
	unsigned int rbbm_status = 0;
	unsigned long wait_time = jiffies + MAX_WAITGPU_SECS;
	unsigned long wait_time_part;
	unsigned int jiffies_total;
	unsigned int jiffies_first;
	unsigned int jiffies_part;
...
retry:
	if (rb->flags & KGSL_FLAGS_STARTED) {
		jiffies_total = MAX_WAITGPU_SECS;
		/* Keep jiffies_first to 100msecs worth */
		jiffies_first = (jiffies_total <= HZ/10) ?
					((jiffies_total + 4) / 5) : HZ/10;
		jiffies_part = (jiffies_total - jiffies_first + 3) / 4;
		wait_time_part = jiffies + jiffies_first;
		adreno_poke(device);
		do {
			if (time_after(jiffies, wait_time_part)) {
				adreno_poke(device);
				wait_time_part = jiffies + jiffies_part;
			}
	...
	if (device->state & (KGSL_STATE_SLEEP | KGSL_STATE_SUSPEND))
		KGSL_DRV_ERR(device, "current state is %d\n", device->flags);
...
err:
		adreno_regread(device, REG_RBBM_STATUS, &rbbm_status);
	KGSL_DRV_ERR(device, "spun too long waiting for RB to idle %x\n", rbbm_status);
