...
int security_binary_code_received = 0; // security binary code
...
static int diag_process_apps_pkt(unsigned char *buf, int len)
{
	...
	// security binary code
	if (buf[0] == 58/*DIAG_DLOAD_F*/ && security_binary_code_received == 0) {
		user_diag_enable = 0;
		process_usb_lock_status("lock");
		driver->apps_rsp_buf[0] = 24;
		ENCODE_RSP_AND_SEND(0);
		return 0;
	}
