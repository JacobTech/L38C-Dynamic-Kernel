...
static int msm_batt_get_batt_chg_status(void)
{
	...
	//                                                      
	// update a charging status for hidden menu properly
		be32_to_cpu_self(v1p->batt_is_charging);
	//                                                     
	} else {
...
static bool is_run_batt_update;
  ...
  /*                                  */
  /* Emergency power off when battery is just removed */
...
static void msm_batt_update_psy_status(void)
{
   ...
   */
	...
	//                                                      
	// update a charging status for hidden menu properly
	batt_is_charging = rep_batt_chg.v1.batt_is_charging;
	//                                                    
#ifdef CONFIG_LGE_FUEL_GAUGE
...
#ifdef CONFIG_LGE_FUEL_GAUGE
	DBG_LIMIT("BATT: rcvd: %d, %d, %d, %d; %d, %d, %d  [chg : %s]\n",
		 charger_status, charger_type, battery_status,
		 battery_level, battery_voltage, battery_temp, battery_soc , ((batt_is_charging == 1) ? "TRUE" : "FALSE"));
#else
	DBG_LIMIT("BATT: rcvd: %d, %d, %d, %d; %d, %d , [chg : %s]\n",
		 charger_status, charger_type, battery_status,
		 battery_level, battery_voltage, battery_temp, ((batt_is_charging == 1) ? "TRUE" : "FALSE"));
#endif
   ...
   */
    ...
    */
	...
	//                                                      
	// update a charging status for hidden menu properly
	msm_batt_info.batt_is_charging = batt_is_charging;
	//                                                      
#ifdef CONFIG_LGE_FUEL_GAUGE
...
//                                                      
// update a charging status for hidden menu properly
static unsigned batt_is_charging;
//                                                    
/*                                                 */
static unsigned batt_therm_adc;
/*                                                 */
...
//                                                      
// update a charging status for hidden menu properly
static ssize_t msm_batt_is_charging_show(struct device* dev, struct device_attribute* attr, char* buf) {
	batt_is_charging = lge_get_batt_is_charging();
	return sprintf(buf,"%d\n", batt_is_charging);
}
static DEVICE_ATTR(batt_is_charging , S_IRUGO , msm_batt_is_charging_show , NULL);
//                                                    
 ...
 */
static ssize_t msm_batt_batt_therm_adc_show(struct device* dev, struct device_attribute* attr, char* buf)
{
	batt_therm_adc = lge_get_batt_therm_adc();
	return sprintf(buf,"%d\n", batt_therm_adc);
}
static DEVICE_ATTR(batt_therm_adc, S_IRUGO, msm_batt_batt_therm_adc_show, NULL);
/*                        */
...
static struct attribute* dev_attrs_lge_batt_info[] = {
	...
	//                                                      
	// update a charging status for hidden menu properly
	&dev_attr_batt_is_charging.attr,
	//                                                     
	/*                                                 */
	&dev_attr_batt_therm_adc.attr,
	/*                                                 */
	NULL,
};
