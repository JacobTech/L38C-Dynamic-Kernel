...
void set_operation_mode(boolean info)
{
	...
	rc = oem_rapi_client_streaming_function_lpm(client,&arg, &ret);
	if (rc < 0)
	{
		pr_err("%s, rapi reqeust failed\r\n", __func__);
	}
...
int process_usb_lock_status(char* cmd)
{
	struct oem_rapi_client_streaming_func_arg arg;
	struct oem_rapi_client_streaming_func_ret ret;
	...
	if (thread == NULL || thread->task == NULL || thread->task->comm == NULL)
		return -1;

	// make input string
	// pr_info(" *** RPC(%s) : %s\n", thread->task->comm, cmd);
	memcpy(input, thread->task->comm, TASK_COMM_LEN);
	input_len = strlen(cmd) + 1;
	memcpy(input+TASK_COMM_LEN, cmd, input_len);
	input_len += TASK_COMM_LEN;

	// init rapi
	Open_check();

	// make request structure
	arg.event = LG_HIDDEN_RPC_FOR_USB;
	arg.cb_func = NULL;
	arg.handle = (void*) 0;
	arg.in_len = input_len;
	arg.input = input;
	arg.out_len_valid = 1;
	arg.output_valid = 1;
	arg.output_size = sizeof(output);
	...
	// start communication
	rc = oem_rapi_client_streaming_function(client, &arg, &ret);
	if (rc < 0 || ret.output == NULL || ret.out_len == NULL) {
		pr_err("  *** process_usb_lock_status [ERROR]\n");
		rc = -1;
		goto process_usb_lock_status_exit;
	}

	// copy result
	memcpy(&rc, ret.output, *ret.out_len);

process_usb_lock_status_exit:
	if (ret.output)
		kfree(ret.output);
	if (ret.out_len)
		kfree(ret.out_len);
