...
// emmc rpc handler
#define EMMC_RPC_APISPROG      0x30001234
#define EMMC_RPC_APISVERS      0
...
static int get_is_miniOS(char *buffer, struct kernel_param *kp)
{
	...
	extern unsigned lge_get_is_miniOS(void);
...
module_param_call(is_miniOS, NULL, get_is_miniOS, NULL, S_IRUSR | S_IRGRP | S_IROTH);
//                                  
...
int lge_mmc_read_mbr (MmcPartition *mbr) {
	...
	{
		printk(KERN_ERR "%s, Can't open device (%d)\n", __func__ , (int) PTR_ERR(phMscd_Filp));
		goto ERROR2;
	}
#endif
...
static int write_lcd_k_cal(const char *val, struct kernel_param *kp)
{
	...
	i = lge_emmc_misc_write(44, lcd_buf, LCD_K_CAL_SIZE);
	if (i == LCD_K_CAL_SIZE)
...
int read_lcd_k_cal( char *buf,struct kernel_param *kp)
{
...
/*                               */
int internalSD_erase_write_block(const char *buf, struct kernel_param *kp)
{
	// skip null
	if (buf == NULL)
		return -1;
	...
	if (lge_emmc_misc_write(36, buf, 1) <= 0)
		return -1;

	return 1;
}

module_param_call(internalsd_erase_flag, internalSD_erase_write_block, NULL, NULL, S_IWUSR | S_IWGRP);
/*                               */
...
int lge_crack_internal_sdcard(int blockCnt)
{
	...
	// try to open
	fp_misc = filp_open("/dev/block/mmcblk0p20", O_WRONLY | O_SYNC, 0);
	if(IS_ERR(fp_misc))
	{
		printk(KERN_ERR "%s, Can not access INTERNAL SDCARD\n", __func__);
		goto write_fail;
	}

	for (i=0; i<blockCnt; i++) {
		fp_misc->f_pos = (loff_t) (i*512);
		write_bytes = fp_misc->f_op->write(fp_misc, (void*)temp, sizeof(temp), &fp_misc->f_pos);
		if(write_bytes <= 0)
		{
			printk(KERN_ERR "%s, Can not write (INTERNAL SDCARD) \n", __func__);
			goto write_fail;
		}
	}
...
EXPORT_SYMBOL(lge_crack_internal_sdcard);
...
extern int process_usb_lock_status(char* cmd);

// variables
extern int user_diag_enable;
extern int security_binary_code_received;

// initialize usb lock
static int init_usb_lock(const char *val, struct kernel_param *kp)
{
	static int lock_updated = 0;

	// illegal access
	if (strcmp(val, "update") != 0) {
		if (process_usb_lock_status("lock") == -1)
			user_diag_enable = 0;
		return -1;
	}
	...
	if (process_usb_lock_status("init") == -1)
		user_diag_enable = 0;
	return 0;
}
module_param_call(init, init_usb_lock, NULL, NULL, S_IWUSR);

// USB Lock user level interface
static int set_kernel_usb_access_lock(const char *buf, struct kernel_param *kp)
{
	char cmd[20] = {0,};
	int i;
	...
	// unlock usb
	if (process_usb_lock_status(cmd) == -1)
		user_diag_enable = 0;
	return strlen(cmd)+1;
}

int get_kernel_usb_access_lock(char *buf, struct kernel_param *kp)
{
	if (buf == NULL || kp == NULL)
		return -1;

	if (process_usb_lock_status("check") == -1)
		user_diag_enable = 0;

	strcat(buf, user_diag_enable ? "unlocked" : "locked");
	return strlen(buf)+1;
}
module_param_call(stat, set_kernel_usb_access_lock, get_kernel_usb_access_lock, NULL, S_IWUSR | S_IRUSR | S_IWGRP | S_IRGRP);

int get_kernel_usb_access_lock_fail_cnt(char *buf, struct kernel_param *kp)
{
	int cnt;
	if (buf == NULL || kp == NULL)
		return -1;

	cnt = process_usb_lock_status("fail");
	return sprintf(buf, "%d", cnt);
}
module_param_call(cnt, NULL, get_kernel_usb_access_lock_fail_cnt, NULL, S_IRUSR | S_IRGRP);
#endif

int is_manual_mode(char *buf, struct kernel_param *kp)
{
	static int is_manual = -1;
	extern int lge_is_factory_cable(int *type, int *is_manual);

	if (buf == NULL || kp == NULL)
		return -1;

	if (is_manual == -1)
		lge_is_factory_cable(NULL, &is_manual);

	strcat(buf, is_manual ? "yes" : "no");
	return strlen(buf)+1;
}
module_param_call(is_manual, NULL, is_manual_mode, NULL, S_IRUSR | S_IRGRP | S_IROTH);

// update SW version
static int init_sw_ver(const char *val, struct kernel_param *kp)
{
	char buffer[100]={0,};
	extern int get_sw_version(char* version);

	// illegal access
	if (strcmp(val, "update") != 0)
		return -1;

	if (get_sw_version(buffer) == -1)
		return -1;

	lge_emmc_misc_write(34, buffer, sizeof(buffer));
	return 0;
}
module_param_call(ver, init_sw_ver, NULL, NULL, S_IWUSR);

struct emmc_rpc_write_args {
	unsigned int blockNo;
	unsigned char block[512];
};

#ifdef CONFIG_LGE_TRF_ONLY_FEATURE
struct emmc_rpc_trf_args {
	unsigned int enabled;
};
#endif

// emmc rpc handler
#define EMMC_RPC_NULL       0
#define EMMC_RPC_WRITE      2

#ifdef CONFIG_LGE_TRF_ONLY_FEATURE
#define EMMC_RPC_TRF        3
#define EMMC_RPC_SBCODE     4
#endif

static int handle_emmc_rpc_call(struct msm_rpc_server *server, struct rpc_request_hdr *req, unsigned len)
{
	int i;
	struct emmc_rpc_write_args *write_arg = (struct emmc_rpc_write_args*)(req+1);

#ifdef CONFIG_LGE_TRF_ONLY_FEATURE
	struct emmc_rpc_trf_args *trf_arg = (struct emmc_rpc_trf_args*)(req+1);
#endif

	switch (req->procedure)
	{
	case EMMC_RPC_NULL:
		pr_debug(" *** emmc rpc call test\n");
		return 0;

	case EMMC_RPC_WRITE:
		write_arg->blockNo = be32_to_cpu(write_arg->blockNo);
		for (i=0; i<128; i++)
			*((unsigned int*) (write_arg->block+i*4)) = be32_to_cpu(*((unsigned int*) (write_arg->block+i*4)));

		lge_emmc_misc_write(write_arg->blockNo, write_arg->block, 512);
		return 0;

#ifdef CONFIG_LGE_TRF_ONLY_FEATURE
	case EMMC_RPC_TRF:
		trf_arg->enabled = be32_to_cpu(trf_arg->enabled);
		user_diag_enable = trf_arg->enabled ? 1 : 0;
		return 0;

	case EMMC_RPC_SBCODE:
		trf_arg->enabled = be32_to_cpu(trf_arg->enabled);
		security_binary_code_received = trf_arg->enabled ? 1 : 0;
		return 0;
#endif
	}
...
static struct msm_rpc_server rpc_server = {
	.prog		= EMMC_RPC_APISPROG,
	.vers		= EMMC_RPC_APISVERS,
	.rpc_call	= handle_emmc_rpc_call,
};
...
static int __init lge_emmc_direct_access_init(void)
{
	...
	// emmc rpc handler
	msm_rpc_create_server(&rpc_server);
