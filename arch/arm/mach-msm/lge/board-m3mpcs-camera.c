...
#if 0//20121204 [ms695] Aptina MT9P017 power sequence change
...
#endif//20121204 [ms695] Aptina MT9P017 power sequence change
...
{
	int rc = 0;
//20121204 [ms695] Aptina MT9P017 power sequence change
...
void camera_power_on_IO_Digital(void)
{
	...
	if (rc < 0)
		pr_err("%s: regulator_set_voltage(ldo4) failed\n",
			__func__);
	rc = regulator_enable(ldo4);//CAM_IO 1.8v
	if (rc < 0)
		pr_err("%s: regulator_enable(ldo4) failed\n",
			__func__);
	...
	if (rc < 0)
		pr_err("%s: regulator_set_voltage(ldo3) failed\n",
			__func__);
	rc = regulator_enable(ldo3);//CAM_AF 2.8v
	if (rc < 0)
		pr_err("%s: regulator_enable(ldo3) failed\n",
			__func__);
...
void camera_power_on_Analog(void)
{
	...
	rc = regulator_enable(ldo2);//CAM_ANALOG 2.8v
	if (rc < 0)
		pr_err("%s: regulator_enable(ldo2) failed\n",
		__func__);
...
//20121204 [ms695] Aptina MT9P017 power sequence change	
	.camera_power_on_IO_Digital   = camera_power_on_IO_Digital,
	.camera_power_on_Analog   = camera_power_on_Analog,
