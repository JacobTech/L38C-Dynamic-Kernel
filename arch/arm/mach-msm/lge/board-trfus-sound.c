...
/* ear sense driver */
#if 1 //HEADSET CHANGE
static struct gpio_h2w_platform_data trfus_h2w_data = {
	.gpio_detect = GPIO_EAR_SENSE,
	.gpio_button_detect = GPIO_BUTTON_DETECT,
};

static struct platform_device trfus_h2w_device = {
	.name = "lge-switch-gpio",
	.id = -1,
	.dev = {
		.platform_data = &trfus_h2w_data,
	},
};
	...
	if (gpio_value == EAR_EJECT) {
		...
		*value = 0;
		...
		if (lge_bd_rev >= LGE_REV_B)
			gpio_value = !gpio_get_value(hook_key_gpios[LGE_REV_B]);
		else
...
static char *m3mpcs_gpio_earsense_print_name(int state)
{
	if (state)
		return "Headset";
	else
...
static int m3mpcs_gpio_hook_key_work_func(int *value)
{
	...
	*value = KEY_MEDIA;
	...
	printk(KERN_INFO "m3mpcs_gpio_hook_key_work_func return value:%d \n",gpio_value);
...
static struct platform_device *trfus_sound_devices[] __initdata = {
	&msm_device_snd,
	&msm_device_adspdec,
	&trfus_h2w_device,
	&asoc_msm_pcm,
...
*/
	platform_add_devices(trfus_sound_devices, ARRAY_SIZE(trfus_sound_devices));
}
