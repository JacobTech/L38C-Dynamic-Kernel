...
static void __init e0eu_init_i2c_sensor(int bus_num)
{
...
#if defined(CONFIG_BACKLIGHT_BU61800)	
	if (onoff) {
		printk(KERN_INFO "LDO1 vreg set.\n");
		rc = bu61800_ldo_enable(NULL,1,onoff);
		if (rc < 0) {
			printk(KERN_INFO "[Proximity] %s: bu61800_ldo_enable(ldo1) ON failed\n", __func__);
		}
		...
		rc = bu61800_ldo_enable(NULL,1,onoff);
		if (rc < 0) {
			printk(KERN_INFO "[Proximity] %s: bu61800_ldo_enable(ldo1) OFF failed\n", __func__);
		}
	}
#endif	
#if defined(CONFIG_BACKLIGHT_RT9396)	
		if (onoff) {
			printk(KERN_INFO "LDO1 vreg set.\n");
			rc = rt9396_ldo_enable(NULL,1,onoff);
			if (rc < 0) {
				printk(KERN_INFO "[Proximity] %s: bu61800_ldo_enable(ldo1) ON failed\n", __func__);
			}
...
void __init lge_add_input_devices(void)
{
	...
	lge_add_gpio_i2c_device(e0eu_init_i2c_sensor);
