...
#ifdef LGE_DEBUG
	printk("%s: open callback received\n", __func__);
#else
...
#ifdef LGE_DEBUG
	printk("open partition %s handle=%d\n", event_args->path, cid);
#else
...
#ifdef LGE_DEBUG
	printk("%s: get err callback received\n", __func__);
#else
...
static int rmt_storage_sdio_smem_probe(struct platform_device *pdev)
{
	...
	if (ret)
#ifdef LGE_DEBUG
		printk("%s: Error (%d) registering sdio_smem client\n",
#else
...
#ifdef LGE_DEBUG
	printk("%s: Alloc rmt buf callback received\n", __func__);
#else
...
#ifdef LGE_DEBUG
	printk("%s: handle=0x%x size=0x%x\n", __func__, handle, size);
#else
...
#ifdef LGE_DEBUG
	printk("%s: %d bytes at phys=0x%x for handle=%d found\n",
#else
			    ...
			    unsigned long arg)
{
...
#ifdef LGE_DEBUG
		printk("%s: get shared memory parameters ioctl\n", __func__);
#else
	...
	case RMT_STORAGE_WAIT_FOR_REQ:
#ifdef LGE_DEBUG
		printk("%s: wait for request ioctl\n", __func__);
#else
		...
		WARN_ON(kevent == NULL);
#ifdef LGE_DEBUG
		printk("%s: Got Event from modem %d \n", __func__,kevent->event.id);
#else
...
#endif
#ifdef LGE_DEBUG
		printk("%s: \thandle=%d err_code=%d data=0x%x\n", __func__,
#else
...
#ifdef LGE_DEBUG
		printk("%s: RAMFS entry: addr = 0x%08x, size = 0x%08x\n",
#else
...
#ifdef LGE_DEBUG
		printk("RAMFS table already filled... skipping %s", \
			__func__);
#else
...
static int rmt_storage_reg_callbacks(struct msm_rpc_client *client)
{
	...
	if (ret)
#ifdef LGE_DEBUG
		printk("%s: Unable (%d) registering aloc_rmt_buf\n",
#else
...
#ifdef LGE_DEBUG
	printk("%s: Remote storage RPC client (0x%x)initialized\n",
#else
...
#ifdef LGE_DEBUG
	printk("%s: Shadow memory at %p (phys=%lx), %d bytes\n", __func__,
#else
